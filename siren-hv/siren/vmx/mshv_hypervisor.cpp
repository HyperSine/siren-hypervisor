...
namespace siren::vmx {
    ...
    mshv_hypervisor_t::mshv_hypervisor_t() noexcept : m_vcpus{} {}

    mshv_hypervisor_t::~mshv_hypervisor_t() {
        if (m_vcpus) {
            stop();
        }
    }
    ...
    [[nodiscard]]
    nt_status mshv_hypervisor_t::intialize() noexcept {
        ...
        unique_npaged<mshv_virtual_cpu_t[]> vcpus;
        {
            auto r = allocate_unique_uninitialized<mshv_virtual_cpu_t[]>(npaged_pool, active_cpu_count());
            if (r.has_value()) {
                vcpus = std::move(r.value());
            } else {
                return r.error();
            }
        }

        for (uint32_t cpu_index = 0; cpu_index < vcpus.get_deleter().count; ++cpu_index) {
            std::construct_at(std::addressof(vcpus[cpu_index]), *this, cpu_index);
        }

        for (uint32_t cpu_index = 0; cpu_index < vcpus.get_deleter().count; ++cpu_index) {
            auto status = vcpus[cpu_index].intialize();
            if (!status.is_success()) {
                return status;
            }
        }

        m_vcpus = std::move(vcpus);
        return nt_status_success_v;
    }

    [[nodiscard]]
    implementation_e mshv_hypervisor_t::get_implementation() const noexcept {
        return implementation_e::X86_VMX_MICROSOFT_HV;
    }

    [[nodiscard]]
    virtual_cpu_t* mshv_hypervisor_t::get_virtual_cpu(uint32_t cpu_index) noexcept {
        return cpu_index < m_vcpus.get_deleter().count ? std::addressof(m_vcpus[cpu_index]) : nullptr;
    }

    [[nodiscard]]
    const virtual_cpu_t* mshv_hypervisor_t::get_virtual_cpu(uint32_t cpu_index) const noexcept {
        return cpu_index < m_vcpus.get_deleter().count ? std::addressof(m_vcpus[cpu_index]) : nullptr;
    }

    [[nodiscard]]
    uint32_t mshv_hypervisor_t::get_virtual_cpu_count() const noexcept {
        return static_cast<uint32_t>(m_vcpus.get_deleter().count);
    }

    void mshv_hypervisor_t::start() noexcept {
        ipi_broadcast([this]() noexcept { get_virtual_cpu(current_cpu_index())->start(); });
    }

    void mshv_hypervisor_t::stop() noexcept {
    ...
    void mshv_virtual_cpu_t::evmcs_setup_segment(const x86::gdtr_t& gdtr, const x86::segment_selector_t& ldtr, auto seg_selector, auto seg_base, auto seg_limit, auto seg_access_rights) noexcept {
    ...
    void mshv_virtual_cpu_t::evmcs_setup_guest() noexcept {
    ...
    void mshv_virtual_cpu_t::evmcs_setup_host() noexcept {
        using namespace siren::x86;
        ...
        host_rip.storage = reinterpret_cast<uintptr_t>(&mshv_vmexit_handler_t::entry_point);
    ...
    void mshv_virtual_cpu_t::evmcs_setup_controls_execution() noexcept {
        using namespace siren::x86;
        ...
        //ctrl_ept_pointer.semantics.pml4_physical_address = address_to_pfn(m_hypervisor.get_ept_manager().get_pml4_physical_address(), on_4KiB_page_t{});
    ...
    void mshv_virtual_cpu_t::evmcs_setup_controls_exit() noexcept {
        using namespace siren::x86;
    ...
    void mshv_virtual_cpu_t::evmcs_setup_controls_entry() noexcept {
        using namespace siren::x86;
    ...
    [[msvc::noinline]]
    void mshv_virtual_cpu_t::launch() noexcept {
        using namespace siren::x86;
    ...
    mshv_virtual_cpu_t::mshv_virtual_cpu_t(mshv_hypervisor_t& hypervisor, uint32_t index) noexcept :
    ...
    mshv_virtual_cpu_t::~mshv_virtual_cpu_t() noexcept {
        if (m_running) {
            invoke_debugger_noreturn();
        }
    }
    ...
    [[nodiscard]]
    nt_status mshv_virtual_cpu_t::intialize() noexcept {
        npaged_allocator<std::byte> npaged_pool{};
        ...
        unique_npaged<msr_bitmap_t> msr_bitmap = nullptr;
        x86::paddr_t msr_bitmap_physical_address = 0;

        unique_npaged<vmexit_stack_t> vmexit_stack = nullptr;
        ...
        run_at_cpu(
            ...
            [&hypercall_page_physical_address, &vp_assist_page_physical_address]() noexcept {
                ...
                if (msr_hypercall.semantics.hypercall_page_enable) {
                    hypercall_page_physical_address = x86::pfn_to_address(msr_hypercall.semantics.hypercall_pfn, x86::on_4KiB_page_t{});
                }

                auto msr_vp_assist_page = x86::read_msr<microsoft_hv::HV_X64_MSR_VP_ASSIST_PAGE>();
                if (msr_vp_assist_page.semantics.enable) {
                    vp_assist_page_physical_address = x86::pfn_to_address(msr_vp_assist_page.semantics.physical_address, x86::on_4KiB_page_t{});
                }
            }
        );
        ...
        if (hypercall_page_physical_address) {
            ...
            invoke_debugger();
#endif
            return nt_status_not_implemented_v;
        }
        ...
        {
            auto r = allocate_unique<msr_bitmap_t>(npaged_pool);
            if (r.has_value()) {
                msr_bitmap = std::move(r.value());
                msr_bitmap_physical_address = get_physical_address(msr_bitmap.get());
            } else {
                return r.error();
            }
        }
    ...
    [[nodiscard]]
    hypervisor_t& mshv_virtual_cpu_t::get_hypervisor() noexcept {
        return m_hypervisor;
    }

    [[nodiscard]]
    const hypervisor_t& mshv_virtual_cpu_t::get_hypervisor() const noexcept {
        return m_hypervisor;
    }

    [[nodiscard]]
    uint32_t mshv_virtual_cpu_t::get_index() const noexcept {
        return m_index;
    }

    void mshv_virtual_cpu_t::start() noexcept {
        using namespace siren::x86;
    ...
    void mshv_virtual_cpu_t::stop() noexcept {
        if (m_running) {
    ...
    [[nodiscard]]
    microsoft_hv::vmx_enlightened_vmcs_t* mshv_virtual_cpu_t::get_enlightened_vmcs() noexcept {
        return m_evmcs_region.get();
    }

    [[nodiscard]]
    const microsoft_hv::vmx_enlightened_vmcs_t* mshv_virtual_cpu_t::get_enlightened_vmcs() const noexcept {
        return m_evmcs_region.get();
    }

    [[nodiscard]]
    const void* mshv_virtual_cpu_t::get_hypercall_page() const noexcept {
        return m_hypercall_page;
    }

    void mshv_virtual_cpu_t::inject_bp_exception() noexcept {
        using namespace siren::x86;
    ...
    void mshv_virtual_cpu_t::inject_ud_exception() noexcept {
        using namespace siren::x86;
    ...
    void mshv_virtual_cpu_t::inject_gp_exception(uint32_t error_code) noexcept {
        using namespace siren::x86;
