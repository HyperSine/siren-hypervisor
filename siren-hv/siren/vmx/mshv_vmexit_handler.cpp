...
namespace siren::vmx {
    void mshv_vmexit_handler_t::advance_rip(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        guest_state->rip += virtual_cpu->get_enlightened_vmcs()->info_vmexit_instruction_length;
    }

    [[nodiscard]]
    bool mshv_vmexit_handler_t::siren_hypercall_echo(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        guest_state->rax = 'srhv';
        advance_rip(virtual_cpu, guest_state);
        return true;
    }

    [[nodiscard]]
    bool mshv_vmexit_handler_t::siren_hypercall_turn_off_vm(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept  {
        x86::cr3_t guest_cr3{ .storage = virtual_cpu->get_enlightened_vmcs()->guest_cr3 };
        ...
        x86::msr_t<x86::IA32_FS_BASE> guest_ia32_fs_base{ .storage = virtual_cpu->get_enlightened_vmcs()->guest_fs_base };
        x86::msr_t<x86::IA32_GS_BASE> guest_ia32_gs_base{ .storage = virtual_cpu->get_enlightened_vmcs()->guest_gs_base };
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler_t::siren_hypercall_ept_commit_1gb_page(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler::siren_hypercall_ept_commit_2mb_page(mshv_virtual_cpu* virtual_cpu, guest_state_t* guest_state) noexcept {
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler::siren_hypercall_ept_commit_4kb_page(mshv_virtual_cpu* virtual_cpu, guest_state_t* guest_state) noexcept {
    //    auto guest_page_base = x86::guest_paddr_t{ guest_state->rcx };
    //    auto host_page_base = x86::guest_paddr_t{ guest_state->rdx };
    //    auto flags = std::bit_cast<dynamic_ept::page_flags_t>(static_cast<uint32_t>(guest_state->r8));
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler::siren_hypercall_ept_uncommit_1gb_page(mshv_virtual_cpu* virtual_cpu, guest_state_t* guest_state) noexcept {
    //    auto guest_page_base = x86::guest_paddr_t{ guest_state->rcx };
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler::siren_hypercall_ept_uncommit_2mb_page(mshv_virtual_cpu* virtual_cpu, guest_state_t* guest_state) noexcept {
    //    auto guest_page_base = x86::guest_paddr_t{ guest_state->rcx };
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler::siren_hypercall_ept_uncommit_4kb_page(mshv_virtual_cpu* virtual_cpu, guest_state_t* guest_state) noexcept {
    //    auto guest_page_base = x86::guest_paddr_t{ guest_state->rcx };
    ...
    //[[nodiscard]]
    //bool mshv_vmexit_handler_t::siren_hypercall_ept_flush(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
    //    auto pml4_physical_address = static_cast<mshv_hypervisor_t&>(virtual_cpu->get_hypervisor()).get_ept_manager().get_pml4_physical_address();
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::siren_hypercall_not_implemented(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::dispatch(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        using namespace siren::x86;

        guest_state->rsp = virtual_cpu->get_enlightened_vmcs()->guest_rsp;
        guest_state->rip = virtual_cpu->get_enlightened_vmcs()->guest_rip;
        guest_state->rflags = rflags_t{ .storage = virtual_cpu->get_enlightened_vmcs()->guest_rflags };

        bool resume;
        vmcsf_t<VMCSF_INFO_EXIT_REASON> info_exit_reason{ .storage = virtual_cpu->get_enlightened_vmcs()->info_exit_reason };
        ...
        if (info_exit_reason.semantics.vm_entry_failure == 0) {
            switch (info_exit_reason.semantics.basic_exit_reason) {
                case x86::vmx_exit_reason_e::CR_ACCESS:
                    return on_cr_access(virtual_cpu, guest_state);
                case x86::vmx_exit_reason_e::INSTRUCTION_VMCALL:
                    resume = on_instruction_vmcall(virtual_cpu, guest_state); break;
                case x86::vmx_exit_reason_e::INSTRUCTION_CPUID:
                    resume = on_instruction_cpuid(virtual_cpu, guest_state); break;
                case x86::vmx_exit_reason_e::INSTRUCTION_HLT:
                    resume = on_instruction_hlt(virtual_cpu, guest_state); break;
                //case x86::vmx_exit_reason_e::INSTRUCTION_INVLPG:
                //    return handle_invlpg(virtual_cpu, guest_state);
                //case x86::vmx_exit_reason_e::INSTRUCTION_WBINVD_OR_WBNOINVD:
                //    return handle_wbinvd(virtual_cpu, guest_state);
                case x86::vmx_exit_reason_e::INSTRUCTION_RDMSR:
                    resume = on_instruction_rdmsr(virtual_cpu, guest_state); break;
                case x86::vmx_exit_reason_e::INSTRUCTION_WRMSR:
                    resume = on_instruction_wrmsr(virtual_cpu, guest_state); break;
                default: 
                    invoke_debugger();
                    advance_rip(virtual_cpu, guest_state);
                    resume = true;
        ...
        if (resume) {
            if (virtual_cpu->get_enlightened_vmcs()->guest_rsp != guest_state->rsp) {
                virtual_cpu->get_enlightened_vmcs()->guest_rsp = guest_state->rsp;
                virtual_cpu->get_enlightened_vmcs()->mshv_clean_fields.semantics.guest_basic = 0;
            }

            virtual_cpu->get_enlightened_vmcs()->guest_rip = guest_state->rip;

            if (virtual_cpu->get_enlightened_vmcs()->guest_rflags != guest_state->rflags.storage) {
                virtual_cpu->get_enlightened_vmcs()->guest_rflags = guest_state->rflags.storage;
                virtual_cpu->get_enlightened_vmcs()->mshv_clean_fields.semantics.guest_basic = 0;
            }
        }
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_cr_access(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        using namespace siren::x86;

        vmcsf_t<VMCSF_INFO_EXIT_QUALIFICATION> info_exit_qualification
            { .storage = virtual_cpu->get_enlightened_vmcs()->info_exit_qualification };
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_instruction_cpuid(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_instruction_rdmsr(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        uint32_t msr_address = static_cast<uint32_t>(guest_state->rcx);
        uint64_t msr_value = 0;

        if (msr_bitmap_t::low_msr_address_interval_v.contains(msr_address)) {
            invoke_debugger();  // should not happen, since we cleared msr bitmap
        } else if (msr_bitmap_t::high_msr_address_interval_v.contains(msr_address)) {
            invoke_debugger();  // should not happen, since we cleared msr bitmap
        } else {
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_instruction_wrmsr(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_instruction_vmcall(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
        using namespace siren::x86;

        segment_selector_t guest_cs_selector{ .storage = virtual_cpu->get_enlightened_vmcs()->guest_cs_selector };
        if (guest_cs_selector.semantics.rpl == 0) {
            ...
            if (guest_state->rax == 'vhrs') {
                switch (guest_state->rbx) {
                    case 0:
                        return siren_hypercall_echo(virtual_cpu, guest_state);
                    case 1:
                        return siren_hypercall_turn_off_vm(virtual_cpu, guest_state);
                    //case 2:
                    //    return siren_hypercall_ept_commit_1gb_page(virtual_cpu, guest_state);
                    //case 3:
                    //    return siren_hypercall_ept_commit_2mb_page(virtual_cpu, guest_state);
                    //case 4:
                    //    return siren_hypercall_ept_commit_4kb_page(virtual_cpu, guest_state);
                    //case 5:
                    //    return siren_hypercall_ept_uncommit_1gb_page(virtual_cpu, guest_state);
                    //case 6:
                    //    return siren_hypercall_ept_uncommit_2mb_page(virtual_cpu, guest_state);
                    //case 7:
                    //    return siren_hypercall_ept_uncommit_4kb_page(virtual_cpu, guest_state);
                    //case 8:
                    //    return siren_hypercall_ept_flush(virtual_cpu, guest_state);
                    default:
                        return siren_hypercall_not_implemented(virtual_cpu, guest_state);
                }
            } else {
                ...
                if (input_value.semantics.fast) {
                    ...
                    guest_state->rax = microsoft_fast_hypercall_ex(virtual_cpu->get_hypercall_page(), input_value, &input_output_block).storage;
                    ...
                    guest_state->rax = microsoft_hypercall(virtual_cpu->get_hypercall_page(), input_value, guest_state->rdx, guest_state->r8).storage;
                }
    ...
    [[nodiscard]]
    bool mshv_vmexit_handler_t::on_instruction_hlt(mshv_virtual_cpu_t* virtual_cpu, guest_state_t* guest_state) noexcept {
